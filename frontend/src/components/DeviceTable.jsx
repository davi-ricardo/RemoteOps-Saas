          ...
          <tbody className="divide-y divide-border">
            {filteredDevices.map(device => (
              <tr key={device.id} className="bg-surface hover:bg-surface-hover transition-colors">
                ...
                <td className="px-6 py-4">
                  <span className={`px-3 py-1 rounded-full bg-surface text-text-secondary text-xs border border-border ${light ? '' : 'border-opacity-50'}`}>
                    {device.group_name || 'Geral'}
                  </span>
                </td>
                <td className="px-6 py-4">
                  <code className={`text-xs text-pink-500 bg-surface px-2 py-1 rounded-lg break-all border border-border ${light ? '' : 'border-opacity-50'}`}>
                    {device.device_id}
                  </code>
                </td>
                <td className="px-6 py-4">
                  <button
                    ...
                    className={`px-4 py-2 text-sm text-primary hover:text-primary-hover hover:bg-primary/10 rounded-xl transition-all border border-primary/20 ${light ? '' : 'border-opacity-50'}`}
                  >
                    Editar
                  </button>
                </td>
